...
# Importamos la librería "shutil" para la gestión de archivos y directorios en el sistema, en este caso para poder borrar de manera recursiva una carpeta y todo su contenido.
...
class GestorArchivos:
    ...
    def InformacionArchivo():
        ...
        if os.path.isfile(info_archivo):
            print("\n=== INFO. DEL ARCHIVO '"+info_archivo+"' ===")

            print("\nTamaño en bytes: ",os.path.getsize(info_archivo))
            print("Ultima fecha de modificacion: ",time.ctime(os.path.getmtime(info_archivo)))
            print("Ultima fecha de acceso: ",time.ctime(os.path.getatime(info_archivo)))
    ...
    def EliminarObjeto():
        ...
        nombre_objeto = input("\nRuta hacia el objeto a eliminar (archivo o carpeta): ")
        ...
        if os.path.isfile(nombre_objeto):
            ...
            if confirmacion.lower() == "si":
                shutil.rmtree(nombre_objeto)
                print("\nLa carpeta '"+nombre_objeto+"' y su contenido ha sido eliminada")
